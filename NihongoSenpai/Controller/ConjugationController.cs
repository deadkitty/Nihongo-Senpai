...
namespace NihongoSenpai.Controller
{
    public static class ConjugationController
    {
        ...
        public static void LoadLessons(List<Lesson> selectedLessons)
        {
            Word.EType[] types = 
            {
                Word.EType.verb1,
            ...
            ConjugationData.Words = DataManager.GetWords(selectedLessons, types);
            
            ConjugationData.ActiveWords = new List<Word>();
        ...
        {        
            ...
            ConjugationData.ItemsLeft = ConjugationData.Words.Count;
        }
        ...
        public static bool[] CheckWords(String[] answers)
        {
            ...
            for (int i = 0; i < answers.Length; ++i)
            {
                ...
                if (ConjugationData.ActiveWords[i].kanji != "")
                {
                ...
                {
                    ConjugationData.TargetWords[i] = GetTargetWord(word, ConjugationData.ActiveWords[i].Type, ConjugationData.TargetForms[i]);
            ...
            ConjugationData.ItemsLeft -= ConjugationData.ActiveWords.Count;
        ...
        {
            ConjugationData.ActiveWords.Clear();

            if (itemIndex >= ConjugationData.Words.Count)
            {
            ...
            else
            {
                ...
                int Count = Math.Min(ConjugationData.maxActiveConjugationWordsCount, ConjugationData.Words.Count - itemIndex);

                for (int i = 0; i < Count; ++i)
                {
                    ConjugationData.ActiveWords.Add(ConjugationData.Words[itemIndex]);
        ...
        public static void InitTargetForms()
        {
            ...
            {
                switch(Util.GetRandomNumber((int)ConjugationData.ETargetForm.count))
                {
